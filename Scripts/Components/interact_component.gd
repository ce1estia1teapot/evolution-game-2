...
The intended use is as follows:
	...
	provided arguments will be a value from Enums.InteractionComponentMode,
	specifying which interact type was used, and a Node corresponding to 
	the interactor that initiated the exchange
...
SUMMARY:
	...
	The component only exposes the ability to display a prompt and accept & hand-off 
	interact actions.
"""
...
func interact(p_interaction_mode: Enums.InteractionComponentMode, p_interactor : PlayerAvatar) -> void:
	
	self.interacted.emit(p_interaction_mode, p_interactor)
